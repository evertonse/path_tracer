...
MyGui :: struct {
    ...
    MAX_REFLECTIONS_RENDER_MODE: int,
    NUMBER_OF_SAMPLES_RENDER_MODE: int,
    MAX_REFLECTIONS_PREVIEW_MODE: int,
    NUMBER_OF_SAMPLES_PREVIEW_MODE: int,

    colourMultiplier:float ,
    focusDistance:  float,
    apertureSize:   float,
    zoom:           float,
    showNormals: bool ,
    onConstantUpdate: bool ,
    // post process: ,
    expousure: float,
    brightness:float ,
    contrast: float,
    ...
    loadImageError :bool ,
    changeSkybox : bool,
    postProcessSettings :bool ,
    isSaveFile : bool,
    isReadFile : bool,
    isMouseControl : bool,
    useSkyboxColour : bool,
    materials : [3]string ,
    selectedItemFromCombo : int,
    nextObjectToAdd : int,
    selectedObjectFromCollection : int,
    selectedMaterialFromCombo : int,
    inputBuffer : [20]u8,
    inputBufferSaveFile: [20]u8,
    previousTime: f64,
    WIDTH, HEIGHT, fps, frameCount : int,
}

mygui_make :: proc(scene_ptr: ^Scene,  skybox_ptr: ^Skybox,  camera_ptr: ^Camera, width, height: i32,  window_ptr : ^glfw.WindowHandle) -> (self: MyGui) {
    using self
    // camera settings
    renderMode = false; // false stands for performance mode, when true for render mode
    MAX_REFLECTIONS_RENDER_MODE = 12;
    NUMBER_OF_SAMPLES_RENDER_MODE = 5;
    MAX_REFLECTIONS_PREVIEW_MODE = 1;
    NUMBER_OF_SAMPLES_PREVIEW_MODE = 1;
    
    colourMultiplier = 1.0;  // colour multiplier when the a ray has reaches the maximum number of reflections
    focusDistance = 3.0;
    apertureSize = 0.0;
    zoom = 1.0;
    
    showNormals = false;
    onConstantUpdate = true;
    ...
    bloomMaxLod = 4;
    numberOfBounces = 4;
    tresholdIntensity = 1.0;
    
    // plane grid settings
    planeGrid = false;
    gridCol2 = { 0.0, 0.0, 0.0 };
    tileSize = 2.0;
    ...
    changeSkybox = false;
    postProcessSettings = false;
    isSaveFile = false;
    isReadFile = false;
    isMouseControl = true;
    useSkyboxColour = false;
    items = { "Sphere", "Cube", "Plane" };
    materials = { "Matte - Specular", "Light source", "Lens" };
    selectedItemFromCombo = 0;
    nextObjectToAdd = 0;
    selectedObjectFromCollection = 0;
    selectedMaterialFromCombo = 0;
    inputBuffer = [20]u8{};  // buffer for an object name
    inputBufferSaveFile = [20]u8{};  // buffer for a scene name
    WIDTH, HEIGHT, fps, frameCount = 0,0,0,0;
    self.previousTime = glfw.GetTime();

    WIDTH = auto_cast width;
    HEIGHT = auto_cast height;
    self.window = window_ptr;
    self.scene  = scene_ptr;
    self.skybox = skybox_ptr;
    self.camera = camera_ptr;
    assert(
        (    skybox != nil
          && window != nil
          && skybox != nil
        ...
        "WTF pointers do be nil, why though?"
    )
...
calcFps :: proc(using self: ^MyGui) {
    currentTime := glfw.GetTime();
	frameCount += 1 ;
	if currentTime - previousTime >= 1.0 {
		self.fps = int(f64(frameCount) / (currentTime - previousTime));
		previousTime = currentTime;
		frameCount = 0;
	}
}

sceneCollectionWindow :: proc(using self: ^MyGui) {
}

inputWindow :: proc(using self: ^MyGui) {
}

skyboxWindow :: proc(using self: ^MyGui) {
}

objectPropertiesWindow :: proc(using self: ^MyGui) {
}

cameraSettingsWindow :: proc(using self: ^MyGui) {
}

screenshotInfo :: proc(using self: ^MyGui) {
}



skyboxError :: proc(using self: ^MyGui) {
}

postProcessWindow :: proc(using self: ^MyGui) {
}

readFileWindow :: proc(using self: ^MyGui) {
}


saveFileWindow :: proc(using self: ^MyGui) { }

renderMyGui :: proc(using self: ^MyGui) {
}


mygui_update :: proc(using self: ^MyGui) {
    calcFps(self)
    calcFps(self)
}
